        ...
        <div className="space-y-6">
            ...
            <div className="flex space-x-2">
                {['all', 'admin', 'hr'].map((f) => (
                    <button
                        ...
                        className={`px-4 py-2 rounded-lg text-sm font-medium capitalize transition-colors ${filter === f ? 'bg-primary text-black' : 'bg-surface border border-border text-text-muted hover:text-text'
                            }`}
                    >
                        {f === 'all' ? 'All Users' : f}s
                    </button>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
                    <div className="bg-surface p-8 rounded-xl border border-border w-full max-w-md relative">
                        ...
                        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                            ...
                            <div>
                                ...
                                <input {...register('email', { required: true })} type="email" className="w-full bg-background border border-border rounded p-2 text-text" />
                            </div>
                            <div>
                                <label className="block text-sm text-text-muted mb-1">Password</label>
                                <input {...register('password', { required: true })} type="password" className="w-full bg-background border border-border rounded p-2 text-text" />
                            </div>
                            <div>
                                <label className="block text-sm text-text-muted mb-1">Role</label>
                                <select {...register('role')} className="w-full bg-background border border-border rounded p-2 text-text">
