        ...
        <section className="section-padding bg-background">
            <div className="container-custom">
                <div className="text-center mb-16">
                    <h2 className="text-3xl md:text-4xl font-bold text-text mb-4">Why Join Aarvion?</h2>
                    <p className="text-text-muted">More than just a job, it's a place to grow.</p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    {perks.map((perk, idx) => (
                        <div key={idx} className="p-6 rounded-xl bg-surface border border-border text-center hover:-translate-y-2 transition-transform duration-300">
                            <div className="text-primary mb-4 flex justify-center">{perk.icon}</div>
                            <h3 className="text-xl font-bold text-text mb-2">{perk.title}</h3>
                            <p className="text-text-muted">{perk.desc}</p>
                        </div>
                    ))}
                </div>
            </div>
